---
name: adhd-mode-off
description: 'Turn adhd-mode off for the current context and return to the default output style. Explicit invocation only; the mode returns on the next session, clear, compaction, resume, or fork.'
...
again, with no notice to the reader. If the style comes back, that is the
mechanism — not the reader being ignored. Say so plainly if they ask.
