...
## 하네스 밖에서 잰 것 (2026-08-27) — Codex

대화형 Codex 세션 두 개가 비용의 전부다. 판정 근거는 모델 자기보고가 아니라 Codex 가
세션 기록(`~/.codex/sessions/**/rollout-*.jsonl`)에 남기는 `last_token_usage.input_tokens` 다.

- **Codex 캐시는 저장소를 따라오지 않는다.** `~/.codex/plugins/cache/` 사본이 `f7671e7`
  이전 상태였다 — 규칙 11개, 지운 off 스킬 잔존, `SubagentStart` 훅 잔존. Claude Code 는
  작업 트리를 직접 읽어 즉시 반영되지만 Codex 는 이 사본을 실행한다. 고친 것이 Codex 에
  닿으려면 `codex plugin add` 를 다시 돌리고 훅을 다시 승인해야 하고, 그 전까지 Codex
  사용자는 옛 결함을 그대로 쓴다. `codex plugin list` 는 양쪽 다 `installed, enabled` 다.
- **`$adhd-mode` 는 Codex 에서 아무 일도 하지 않는다.** 같은 첫 메시지를 프리픽스만 달리해
  새 세션 두 개에 보냈다 — 22,220 대 22,223 토큰. 차이 3은 그 글자값 자체다. 룰셋 한 벌이
  더 들어갔다면 1,900대가 보였어야 한다.
- **스킬이 어느 호스트의 카탈로그에도 없다.** 그 Codex 세션의 스킬 목록 34개에 `adhd-mode`
  는 언급 0회다. 위 결과의 원인은 중복 감지가 아니라 미등록이다. Claude 쪽도 같다 —
  `disable-model-invocation` + `user-invocable: false` 조합에서 세션 스킬 목록에 나타나지
  않는다. `claude plugin details` 의 `Always-on: ~103 tok` 은 정적 추정치다.
- **훅 런처 수정이 Codex 에서도 동작한다.** 캐시 갱신·재승인 뒤 두 세션 모두 배너가 떴고
  주입된 본문은 현재 판이었다 (지운 옛 규칙 8 `Matter-of-fact tone` 0회).

**남는 부채:** 위 예시 수정으로 룰셋이 바뀌었으므로, 기존 규칙별 판정은 엄밀히는 지금과
조금 다른 룰셋을 잰 것이다. 2026-08-27 의 우선순위 문장 추가(규칙 4 가
규칙 9 를 삼키지 않는다, 옵션 상한은 규칙 9 한 곳에서만, 오버라이드 6 이 오버라이드 2 를
덮지 않는다)도 같은 부채에 들어간다. 재기준선은 하지 않았다 — 이 저장소의 목표가 모든
규칙에 n=5 근거를 갖추는 것이 아니고, 값도 실측으로 확인됐다: 이 디렉터리의 과거 실행
비용에서 역산하면 `--isolate --repeat 5` 전면 실행 404회는 $15~46 이다.
